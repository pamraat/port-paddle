...
contains
   ...
   subroutine simulation_init
      ...
      create_and_initialize_vof: block
         ...
         use vfs_class, only: lvira,VFhi,VFlo,neumann
      ...
      initialize_df: block
         ...
         integer :: i,j,k,np,npline
         real(WP) :: amp,freq,theta,r,quo,rem
         ...
         call param_read('Rotation per sec',angVel)
         angVel=angVel*twoPi
         call param_read('Cylinder position',poscyl)
         ...
         if (df%cfg%amRoot) then
            ...
            do i=1,np/2
               ...
               df%p(i)%vel=0.0_WP
               ! Set position
               npline=int(np/(2*freq))
               quo=int((i-1)/npline)
               rem=mod(i-1,npline)
               theta=quo*twoPi/freq
               r=0.5_WP*Dcyl+(rem+1)*amp/npline
               df%p(i)%pos=[r*cos(theta),r*sin(theta),0.0_WP]
               ! Assign element area
               df%p(i)%dA=twoPi*r/real(np,WP)*df%cfg%zL
               ! Assign outward normal vector
               df%p(i)%norm=df%p(i)%pos/r
               ! Shift cylinder
               df%p(i)%pos=df%p(i)%pos+poscyl
            ...
            do i=(np/2+1),np
               ...
               df%p(i)%vel=0.0_WP
               ! Set position
               theta=real(i-1,WP)*twoPi/real(np/2,WP)
               r=0.5_WP*Dcyl
               df%p(i)%pos=[r*cos(theta),r*sin(theta),0.0_WP]
               ! Assign element area
               df%p(i)%dA=twoPi*r/real(np,WP)*df%cfg%zL
         ...
         do k=df%cfg%kmin_,df%cfg%kmax_
            do j=df%cfg%jmin_,df%cfg%jmax_
               do i=df%cfg%imin_,df%cfg%imax_
                  theta=arctan(df%cfg%xm(i)-poscyl(1),df%cfg%ym(j)-poscyl(2))
                  r=0.5_WP*Dcyl+amp*sin(freq*theta)
                  df%G(i,j,k)=sqrt((df%cfg%xm(i)-poscyl(1))**2+(df%cfg%ym(j)-poscyl(2))**2)-r
               end do
            end do
         end do
   ...
   subroutine simulation_run
      ...
      do while (.not.time%done())
         ...
         ! Impart momentum to the IB particles
         ib_moving: block
            integer  :: i
            real(WP) :: rad
            real(WP), dimension(3) :: r,theta
            do i=1,df%np_
               r=df%p(i)%pos-poscyl
               rad=norm2(r)
               r=r/rad
               theta=[-r(2),r(1),r(3)]
               df%p(i)%vel=angVel*rad*theta
               df%p(i)%pos=df%p(i)%pos+rad*sin(angVel*time%dt)*theta-rad*(1-cos(angVel*time%dt))*r
               df%p(i)%ind=df%cfg%get_ijk_global(df%p(i)%pos,df%p(i)%ind)
            end do
            call df%sync()
            call df%update_VF()
         end block ib_moving
         ...
         remove_vof: block
            ...
            do n=1,vof_removal_layer%no_
               vf%VF(vof_removal_layer%map(1,n),vof_removal_layer%map(2,n),vof_removal_layer%map(3,n))=0.0_WP
            end do
